...
>
  ...
  <button
    ...
    className="absolute top-1/2 left-0 -translate-y-1/2 bg-white p-2 rounded-full shadow hover:bg-gray-100 z-50"
  >
  ...
  <button
    onClick={() =>
      setActiveIndex((prev) =>
        Math.min(prev + 1, groupProducts(msg.matched_products, 4).length - 1)
      )
    }
    className="absolute top-1/2 right-0 -translate-y-1/2 bg-white p-2 rounded-full shadow hover:bg-gray-100 z-50"
  >
      ...
      >
        <div className="flex gap-4 px-2 w-max">
          {group.map((product, idx) => (
            <div
              key={idx}
              className="block w-[260px] border border-indigo-200 rounded-lg p-3 shadow-sm bg-white hover:shadow-md transition"
            >
